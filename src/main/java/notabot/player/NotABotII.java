...
public abstract class NotABotII extends IIGamer {
	...
	public Move iiselectMove(long timeout)
		...
		/*System.out.println("NotABotII: iiselectMove: ");
		for (See see : getCurrentSees().getSees()) {
			System.out.println("NotABotII: iiselectMove: " + see.getContents().toString());
		}*/
		// update time at which timeout will occur
		this.timeout = timeout - TIME_CUSHION;
		// while (!hasTimedOut()) {}
	...
	public static boolean hasTimedOut(){
		return NotABotII.timeout < System.currentTimeMillis();
	}

	public static long timeLeft(){
		return Math.max(0, NotABotII.timeout-System.currentTimeMillis());
	}
