...
public class NotABotIIPropNetStateMachine extends IIStateMachine {
...
//		iipropNet.renderToFile("/Users/hardiecate/Documents/propnetviz.dot");
	...
	public List<Move> getLegalMoves(SeesState seesState, Move move) throws MoveDefinitionException {
		// need to look at updating match's move history
		// handle unknown props differently in propMark(): if they're unknown,
		// keep going down and see if we reach any true props
		if (seesState.getRole().toString().equals("robot")) {
			System.out.println("Previous move: " + move.toString() + '\n');
		}
		...
		for (Proposition prop : iipropNet.getLegalPropositions().get(seesState.getRole())) {
			if (propMark(prop) == 1) {
				...
				moves.add(m);
				if (seesState.getRole().toString().equals("robot")) {
					System.out.println("LEGAL MOVES: " + m.getContents().toString());
				}
			}
		}
		return moves;
	...
	public List<Move> getLegalMoves(SeesState seesState) throws MoveDefinitionException {
		...
		for (Proposition prop : iipropNet.getLegalPropositions().get(seesState.getRole())) {
			if (propMark(prop) == 1) {
				Move m = new Move(prop.getName().getBody().get(1));
				moves.add(m);
				System.out.println(seesState.getRole().toString() + ": " + m.getContents().toString());
			}
		}
		return moves;
	}
	...
	private void propagateSeesHelper(Component c, boolean value) {
		...
		//System.out.println(c.toString() + ": " + iivalue);
	...
	private double propMark(Component c){
		...
		if (c instanceof Proposition){
			...
			// if this value is already set, we can just return it. This has to be done differently
			// than before because propagation might not have extended beneath this prop.
